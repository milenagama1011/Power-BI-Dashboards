# Dashboard Catalog

## Dashboard 1: Academic Performance & Engagement Dashboard

### Overview
Interactive dashboard tracking programme performance and student engagement across 45 academic programmes to support institutional leadership decisions and resource planning.

### 🔗 Access Dashboard
**[Open Dashboard](https://app.powerbi.com/reportEmbed?reportId=d20f06b9-b494-4f64-87c8-9197600e39ec&autoAuth=true&ctid=1eb34f73-38dd-42db-98db-53b54e749d52<img width="468" height="43" alt="image" src="https://github.com/user-attachments/assets/57b531e3-fdba-472e-baf2-91d67e4a2aed" />)**

### 📊 Key Metrics Tracked
- **Participation Rates** - Overall student enrollment and attendance metrics
- **Completion Rates** - Programme completion and graduation statistics
- **Engagement Indicators** - Student interaction and involvement levels
- **Programme Performance Comparisons** - Relative performance across all 45 programmes
- **Trends Over Time** - Historical performance patterns and trajectory analysis

### 💾 Data Sources
- Excel spreadsheets
- Institutional datasets
- Power Query transformations for data cleansing and enrichment

### 🎛️ Interactive Features
- **Filters by Department & Programme** - Drill down into specific academic areas
- **Drill-through Analysis** - Navigate from summary views to detailed records
- **KPI Scorecards** - At-a-glance performance indicators
- **Time-Series Trends** - Historical data visualization and forecasting

### 📈 Business Impact
✅ Enabled leadership to quickly identify underperforming areas  
✅ Prioritized targeted improvements based on data insights  
✅ Supported data-driven policy decisions  
✅ Streamlined resource allocation across programmes  

### 🛠️ Technical Implementation
- **Data Transformation:** Power Query for ETL processes
- **Calculations:** DAX formulas for KPI calculations
- **Visualization:** Multiple chart types for different analytical perspectives
- **Interactivity:** Slicers, filters, and drill-through pages for exploration

---

## Dashboard 2: FIFA Tournament Performance Dashboard

### Overview
Data storytelling dashboard analyzing team and player performance statistics to demonstrate advanced visualization, filtering, and interactive reporting techniques. This showcase highlights the ability to create engaging, user-friendly analytics for non-technical audiences.

### 🔗 Access Dashboard
**[Open Dashboard](https://app.powerbi.com/reportEmbed?reportId=b0e43b79-2567-478a-981f-e56a864d27d8)**

### 📊 Key Metrics Tracked
- **Goals & Assists** - Individual and team scoring performance
- **Team Rankings** - Competitive standings and performance hierarchy
- **Match Outcomes** - Win/loss records and performance patterns
- **Player Performance Stats** - Individual player statistics and contributions

### 💾 Data Sources
- Public FIFA tournament datasets
- Excel data files
- Power Query transformations and data cleaning

### 🎛️ Interactive Features
- **Dynamic Slicers** - Filter by team, player, timeframe, and other dimensions
- **Drill-through Pages** - Deep dive into specific teams or player details
- **Comparative Charts** - Side-by-side performance comparisons
- **Interactive Visuals** - Click-through analysis and dynamic filtering

### 📈 Business Impact
✅ Demonstrates advanced Power BI storytelling capabilities  
✅ Showcases user-friendly dashboard design for non-technical audiences  
✅ Illustrates best practices in data visualization and UX  
✅ Proves ability to communicate complex data in an engaging manner  

### 🛠️ Technical Implementation
- **Visualization Design:** Advanced chart selection and color theory
- **Interactivity:** Dynamic slicers and cross-filtering
- **User Experience:** Intuitive navigation and clear data hierarchy
- **Data Storytelling:** Narrative-driven dashboard layout

---

## 🔧 Technologies & Tools Used

- **Power BI Desktop & Service** - Dashboard development and publishing
- **Power Query** - Data extraction, transformation, and loading (ETL)
- **DAX (Data Analysis Expressions)** - Complex calculations and measures
- **Excel** - Data source and preparation
- **Python** - Data preprocessing and automation (where applicable)
- **SQL** - Database querying (where applicable)

---

## 💡 Key Takeaways

These dashboards demonstrate:
- ✨ **Clear Visual Hierarchy** - Information organized for quick decision-making
- 🎯 **Actionable Insights** - Data presented with business impact in mind
- 🔍 **Interactivity** - Users can explore data at their own pace
- 📱 **Responsive Design** - Accessible across devices
- 🎨 **Professional Aesthetics** - Polished, enterprise-ready appearance

---

For more information or to discuss these projects, please reach out:
- **Email:** milegama1011@gmail.com
- **LinkedIn:** [linkedin.com/in/milenagama](https://www.linkedin.com/in/milenagama)
